...
'Risk Grading':[select('select#onBoardingValue',e.onboarding,'Onboarding'),select('select#geoRiskClient',e.residence,'Residence'),select('select#highOfficial',e.pep,'PEP'),select('select#closeHighOfficial',e.pepRelated,'PEP related'),select('select#isClientIp',e.ip,'IP'),select('select#productTypes',e.product,'Product'),{type:'radio',selector:'input#profession',value:e.occupation?'true':'',label:'Profession category'},select('select#professionName',e.occupation,'Occupation'),select('select#yearlyTransaction',e.transactions,'Yearly transactions'),select('select#hasSourceOfFunds',e.sourceCredible,'Credible source of funds')].filter(action=>action.value),
