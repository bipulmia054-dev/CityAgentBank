...
One website header, a loading screen and a Reload button in the app header. Reload waits for draft writes. Android Back moves through the in-app pages first and closes only from a dashboard. The hash route and username-scoped IndexedDB restore the current page and unfinished form, including images. Clear removes the local draft. Uninstall or clearing site data removes drafts.
