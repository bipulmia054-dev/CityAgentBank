...
async function makePassportPhoto(imageData, extraPrompt = "") {
  const response = await fetch("/api/passport-photo", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ image: imageData, extraPrompt }),
  });
  const result = await readJson(response);
...
const hasAdminAccess = auth => isAdminRole(auth?.role) || auth?.canAdmin === true;
const isCollectionRole = role => ["worker", "area_manager"].includes(role);

function SectionTabs({ label, value, onChange, items }) {
  return <nav className="sectionTabs" aria-label={label}>{items.map(([key, title, count]) =>
...
function AdminUserControl({ users, reload, master, onOpenCustomer }) {
  ...
  const listedUsers = users.filter(item => userTab !== "pending" || item.status === "pending");
  async function toggleAdminAccess(item) {
    const enabled = !item.admin_access;
    if (!confirm(enabled ? `${item.full_name}-কে Admin Access দেবেন? তিনি সব customer, user ও finance পরিচালনা করতে পারবেন।` : `${item.full_name}-এর Admin Access বন্ধ করবেন?`)) return;
    setChanging(item.id);
    try {
      const response = await fetch(`/api/admin/users/${item.id}`, {method:"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify({adminAccess:enabled})});
      const result = await readJson(response);
      if (!response.ok) throw new Error(result.error);
      await reload();
      setFeedback({ok:true, text:enabled ? "Admin Access চালু হয়েছে। Manager অ্যাপ refresh করলে switch button পাবেন।" : "Admin Access বন্ধ হয়েছে।"});
  ...
  return <>
    ...
    <SectionTabs label="User sections" value={userTab} onChange={setUserTab} items={[["pending","Pending Users",users.filter(u=>u.status==="pending").length],["all","All Users",users.length],["managers","Area Managers",users.filter(u=>u.role==="area_manager").length],["create","New Account"],["referrals","Referral Tree"]]} />
    <div className="adminUserGrid">
      {userTab === "create" && <form className="portalPanel adminCreateUser" onSubmit={createUser}><div className="panelTitle"><div><small>ACCOUNT CONTROL</small><h2>নতুন account তৈরি করুন</h2></div><ShieldCheck /></div><label><span>পূর্ণ নাম</span><input required value={form.fullName} onChange={e => setForm({ ...form, fullName: e.target.value })} /></label><label><span>Username</span><input required value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} /></label><label><span>Password</span><input required minLength="8" type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /></label><label><span>Account type</span><select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })}><option value="worker">Worker</option>{master && <option value="subadmin">Subadmin</option>}</select></label><button className="primary full"><Plus /> Account Save করুন</button></form>}
      {userTab === "referrals" && <section className="portalPanel adminReferralPanel"><div className="panelTitle"><div><small>REFERRAL NETWORK</small><h2>Referral Tree</h2></div><span className="codeBadge">MASTER: {"MASTER-BIPUL"}</span></div><p className="secureNotice">Master referral code: <b>MASTER-BIPUL</b></p><Tree /></section>}
    </div>
    {userTab === "managers" && <section className="portalPanel"><div className="panelTitle"><div><small>AREA MANAGER CONTROL</small><h2>Temporary Area Manager</h2></div></div><p className="secureNotice">Area Manager নিজের customer collection করতে পারবেন এবং নিজের নিচের team দেখতে পারবেন। Admin Access চালু থাকলে সব admin details-ও দেখতে পারবেন।</p>{users.filter(item=>item.role==="area_manager").map(item=><p className="ledgerRow" key={item.id}><span><b>{item.full_name}</b><small>{item.phone||"—"} • {item.role === "area_manager" ? `Area Manager • ${item.commission_enabled ? `${item.commission_percent}% commission ON` : "commission OFF"}` : "Worker"}</small></span>{item.role === "area_manager"?<span className="rowActions">{master && <button disabled={changing !== null} onClick={()=>toggleAdminAccess(item)}>{item.admin_access ? "Admin Access বন্ধ" : "Admin Access দিন"}</button>}<button onClick={()=>setManagerCommission(item)}>Commission সেটিং</button><button onClick={()=>configureAreaManagerFor(item)}>Manager বন্ধ</button></span>:<button className="recordEdit" onClick={()=>configureAreaManagerFor(item)}>Area Manager চালু</button>}</p>)}{!users.some(u=>u.role==="area_manager") && <p className="empty">কোনো Area Manager নেই। All Users থেকে manager চালু করুন।</p>}</section>}
    {["all","pending"].includes(userTab) && <section className="portalPanel"><div className="panelTitle"><div><small>ALL ACCOUNTS</small><h2>{userTab === "pending" ? "Pending Users" : "All Users"}</h2></div></div><div className="portalTable">{listedUsers.map(item => <article key={item.id}><div><b>{item.full_name}</b><small>@{item.username} • {item.role}</small></div><div><b>{item.referral_code}</b><small>{item.phone || "Phone নেই"}</small></div><span className={`statusTag ${item.status}`}>{statusLabel[item.status] || item.status}</span><div className="rowActions">{item.role === "worker" && item.status === "pending" && <><button className="workerApprove" disabled={changing !== null} onClick={() => reviewWorker(item,"approved")}><Check/> {changing === item.id ? "অপেক্ষা করুন…" : "অনুমোদন করুন"}</button><button className="danger" disabled={changing !== null} onClick={() => reviewWorker(item,"rejected")}><X/> বাতিল করুন</button></>}{item.role === "worker" && item.status === "approved" && <button onClick={()=>configureAreaManagerFor(item)}>Area Manager চালু</button>}<button onClick={() => openProfile(item.id)}><UserRound /> Profile দেখুন</button></div></article>)}</div>{!listedUsers.length && <p className="empty">এই tab-এ কোনো user নেই।</p>}</section>}
...
function AdminIdentityPdfPreview({ person, label }) {
  const [pdf, setPdf] = useState("");
  useEffect(() => {
    let disposed = false;
    const document = identityPdf(person);
    if (!document) { setPdf(""); return () => { disposed = true; }; }
    asDataUrl(document).then(value => { if (!disposed) setPdf(value); }).catch(() => { if (!disposed) setPdf(""); });
    return () => { disposed = true; };
  }, [person.idFront, person.idBack, person.birthCertificate, person.identityType, person.name, person.nameBn, person.role]);
  if (!pdf) return null;
  return <section className="idPdfPreview adminIdentityPdfPreview">
    <div className="pdfPreviewHead"><div><h3>{label} PDF</h3><small>Front ও Back • A4</small></div></div>
    <PdfEmbed data={pdf}><a href={pdf} target="_blank" rel="noreferrer">ID Card PDF খুলুন</a></PdfEmbed>
    <PdfActions data={pdf} filename={`${personBase(person)}_${person.identityType === "birth" ? "Birth_Certificate" : "ID_Card"}.pdf`} />
  </section>;
}

function AdminReviewMedia({ person, label }) {
  const first = [[person.idFront, "NID Front"], [person.idBack, "NID Back"]];
  const rest = [[person.birthCertificate, "Birth Certificate"], [person.photo, "Passport Photo"]];
  return <details className="reviewMedia"><summary>ছবি ও ID files দেখুন</summary><div className="adminIdentityGrid">
    {first.filter(([src]) => src).map(([src, name]) => <PreviewImage key={name} src={src} label={name} />)}
    <AdminIdentityPdfPreview person={person} label={label} />
    {rest.filter(([src]) => src).map(([src, name]) => <PreviewImage key={name} src={src} label={name} portrait={name === "Passport Photo"} />)}
  </div></details>;
}
...
function AdminPortal({ openRecords, actions, master }) {
  ...
  const visibleCases = cases => cases.filter(item => applicationMatches(item,caseTab) && applicationSearchMatches(item,caseSearch));
  ...
  if(tab==='users')return <div className="adminPortal usersOnly"><button className="secondary" onClick={()=>setTab('overview')}><ChevronLeft/> Admin Overview</button><div className="rowActions">{actions}</div><AdminUserControl users={users} reload={load} master={master} onOpenCustomer={openCase}/>{selectedCase&&<AdminCaseModal data={selectedCase} close={()=>setSelectedCase(null)} reload={load}/>}</div>;
...
function App() {
  ...
  useEffect(() => {
    ...
    readDraft(auth.username).then(draft => {
      ...
      const requested = Number(readPage("page", draft?.step ?? (hasAdminAccess(auth) ? 6 : 0)));
      setStep([0,1,2,3,4,5,6].includes(requested) && (hasAdminAccess(auth) || ![5,6].includes(requested)) ? requested : (hasAdminAccess(auth) ? 6 : 0));
      setDraftReady(true);
  ...
  useEffect(() => {
    window.documentStudioGoBack = () => {
      ...
      if (step === 4) { setStep(hasAdminAccess(auth) ? 6 : 0); return true; }
      if (step === 5) { setStep(hasAdminAccess(auth) ? 6 : 1); return true; }
      if (step === 1) { setStep(hasAdminAccess(auth) ? 6 : 0); return true; }
      return false;
    };
  ...
  useEffect(() => {
    if (draftReady && step === 3 && hasAdminAccess(auth) && !Object.keys(idPdfPreviews).length) openPreview();
  }, [draftReady]);
    ...
    return (
      <Access
        ...
        onAccess={(result) => {
          ...
          setStep(hasAdminAccess(result) ? 6 : 0);
        }}
      />
    );
  ...
  const frameProps = inPortal ? {} : {
    ...
    actions,
    active: step === 5 ? 5 : 1,
    menu: [[hasAdminAccess(auth) ? 6 : 0, Home, "Dashboard"], [1, Plus, "New Customer"], ...(hasAdminAccess(auth) ? [[5, Database, "Customer Files"]] : [])],
    onNavigate: setStep,
  };
  return (
    <CollectionOnly.Provider value={false}>
      <WorkerRoleContext.Provider value={isCollectionRole(auth?.role) && ![5,6].includes(step)}>
      ...
      <div className="appPageContents">
        ...
        {step === 0 ? (
          ...
          <AdminPortal openRecords={() => setStep(5)} actions={actions} master={auth.role === "master_admin"} />
        ) : step === 5 ? (
          ...
          <div className="success">
            ...
            {hasAdminAccess(auth) && <button className="primary" onClick={downloadSaved}>
